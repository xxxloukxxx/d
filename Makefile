...
install:
	...
	sudo apt -qq -y install micro nnn vim zsh greetd dwm stterm xorg okular zathura chromium chromium-l10n suckless-tools aptitude
	...
	sudo apt -qq -y install fzf fd-find bat tree ripgrep btop locales-all build-essential gcc rsstail slock trash-cli
